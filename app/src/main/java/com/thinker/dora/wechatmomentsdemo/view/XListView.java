...
public class XListView extends ListView implements OnScrollListener {
	...
	public void autoRefresh() {
		...
		if (mEnablePullRefresh && !mPullRefreshing) {
			...
			if (mHeader.getVisibleHeight() > mHeaderHeight) {
				mHeader.setState(XHeaderView.STATE_READY, 0);
			} else {
				mHeader.setState(XHeaderView.STATE_NORMAL, 0);
			}
		}

		mPullRefreshing = true;
		mHeader.setState(XHeaderView.STATE_REFRESHING, 0);
		refresh();
	}
	...
	private void updateHeaderHeight(float delta, float Y) {
		mHeader.setVisibleHeight((int) delta + mHeader.getVisibleHeight());

		if (mEnablePullRefresh && !mPullRefreshing) {
			// update the arrow image unrefreshing
			if (mHeader.getVisibleHeight() > mHeaderHeight) {
				mHeader.setState(XHeaderView.STATE_READY, Y);
			} else {
				mHeader.setState(XHeaderView.STATE_NORMAL, Y);
			}
		}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0
				...
				updateHeaderHeight(deltaY / OFFSET_RADIO, ev.getY());
				invokeOnScrolling();
				downPulling();
...
//		return super.onTouchEvent(ev);
	...
	private void downPulling() {
		if (mEnablePullRefresh && null != mListener) {
			mListener.downPulling();
		}
	}
